...
if len(argv) == 3 + 1:  # Использовать файл мира
    ...
    snakelength = int(input("Enter how long snake (1-10)> "))
    if snakelength < 1:
        raise ValueError("Use other snake length!")

    SpdOfGame = int(input("Enter how fast game (20-25)> "))
    world_type = input("Enter filename of world (leave empty for random)> ")
    if len(world_type) != 0:
        FoodMany = None
        BricksMany = None
        world = TorWorld(world_type)
    else:
        widhtWorld = int(input("Enter widht of world (50-80)> "))
        heigthWorld = int(input("Enter heigth of world (50-70)> "))
        FoodMany = int(input("Enter how many food (5-10)> "))
        BricksMany = int(input("Enter how many bricks (1-5)> "))
...
def ShowWorld(world, screen, scale, images):
    for coords, ID in world.GetDiff().items():
        x, y = coords
        idPicture = ID
...
startSnake = [[i, 0] for i in range(snakelength)]
...
print('Time: {} seconds'.format(round(time.time() - startTime)))
